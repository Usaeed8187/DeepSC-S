...
if __name__ == "__main__":
    ...
    for db in range(0,20):
        ...
        sem_enc = tf.keras.models.load_model(sem_enc_h5)
                
        # load channel encoder
        chan_enc_h5 = saved_model_dir + "/chan_enc.h5"
        chan_enc = tf.keras.models.load_model(chan_enc_h5 )
                
        # load channel_decoder
        chan_dec_h5 = saved_model_dir + "/chan_dec.h5"
        chan_dec = tf.keras.models.load_model(chan_dec_h5)
                
        # load semantic_decoder
        sem_dec_h5 = saved_model_dir + "/sem_dec.h5"
        sem_dec = tf.keras.models.load_model(sem_dec_h5)
    ...
    np.save('./test_results.npy',score)
